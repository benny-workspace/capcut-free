...
function evalTransform(clip, tLocal) {
  const kf = clip.keyframes;
  if (!kf) return clip.transform;
  const out = { ...clip.transform };
  for (const prop of ["x", "y", "scale", "rotation", "opacity"]) {
    const list = kf[prop];
...
function needsFramePipe(project) {
...
const MAIN_TRACK_ID = "main";
function newProject(name, width, height) {
...
function mediaUrl(path) {
  return "media:///" + encodeURI(path.replace(/\\/g, "/")).replace(/#/g, "%23").replace(/\?/g, "%3F");
}
...
function renderTextPngDataUrl(clip, W, H) {
...
const PLAYABLE_VCODECS$1 = ["h264", "vp8", "vp9", "av1"];
function proxyPending(type, vcodec, proxyPath) {
  return type === "video" && !proxyPath && !PLAYABLE_VCODECS$1.includes(vcodec || "");
}
...
function transitionAlphas(project, t) {
...
function captionClipsFromWords(words, big, mkId) {
  const groups = [];
  let cur = [];
...
function replaceTrackClips(project, trackId, clips) {
  return {
    ...project,
    tracks: project.tracks.map((t) => t.id === trackId ? { ...t, clips } : t)
  };
}
function packedMain(project) {
  const main = project.tracks.find((t2) => t2.id === MAIN_TRACK_ID);
...
const pushUndo = (s) => [...s.undoStack.slice(-59), s.project];
const useEditor = create((set, get) => ({
  ...
  proxyReady: (mediaId, proxyPath) => set((s) => ({
    project: {
      ...s.project,
  ...
  updateClip: (clipId, patch, withUndo = true) => set((s) => {
  ...
  reorderMain: (clipId, newIndex) => set((s) => {
    const main = s.project.tracks.find((t) => t.id === MAIN_TRACK_ID);
    if (!main) return {};
    const idx = main.clips.findIndex((c2) => c2.id === clipId);
    if (idx < 0) return {};
  ...
  moveToTrack: (clipId, trackId, start, mainIndex) => set((s) => {
    const loc = findClip(s.project, clipId);
    if (!loc) return {};
    if (loc.track.id === trackId && trackId !== MAIN_TRACK_ID) {
      const clips = loc.track.clips.map(
        (c) => c.id === clipId ? { ...c, start: Math.max(0, start) } : c
      );
    ...
    if (!target) return {};
    const moved = { ...loc.clip, start: Math.max(0, start) };
  ...
  setMediaMatte: (mediaId, mattePath) => set((s) => ({
    project: {
      ...s.project,
  ...
  clearKeyframes: (clipId) => set((s) => {
    ...
    if (!loc) return {};
    const clips = loc.track.clips.map(
      (c) => c.id === clipId ? { ...c, keyframes: void 0 } : c
    );
    return {
      project: replaceTrackClips(s.project, loc.track.id, clips),
      undoStack: pushUndo(s),
  ...
  replaceTimeline: (tracks, media) => set((s) => ({
    project: {
      ...s.project,
      tracks,
  ...
  beginInteraction: () => set((s) => ({ undoStack: pushUndo(s), redoStack: [] })),
  splitAtPlayhead: () => set((s) => {
    const ph = s.playhead;
    let loc = null;
    if (s.selectedClipId) {
    ...
    const first = { ...loc.clip, duration: off, transitionAfter: void 0 };
    const second = {
      ...loc.clip,
      id: uid("c"),
  ...
  setPlayhead: (t) => set((s) => ({ playhead: clamp(t, 0, Math.max(projectDuration(s.project), 0)) })),
  setPlaying: (playing) => set({ playing }),
  setPps: (v) => set({ pps: clamp(v, 8, 300) }),
  toggleSnap: () => set((s) => ({ snap: !s.snap })),
  setProjectMeta: (patch) => set((s) => ({
...
function baseClip(mediaId, start, duration, srcIn) {
  return {
    id: uid("c"),
    kind: "video",
    mediaId,
...
function AutoEditDialog({ onClose }) {
  const media = useEditor((s) => s.project.media);
  const sys = useEditor((s) => s.sys);
  const videos = media.filter((m) => m.type === "video");
  const audios = media.filter((m) => m.type === "audio");
  const [sel, setSel] = reactExports.useState(new Set(videos.map((v) => v.id)));
  const [musicId, setMusicId] = reactExports.useState(audios[0]?.id ?? "");
  const [style, setStyle] = reactExports.useState("talking");
  ...
  const run = async () => {
    ...
    try {
      ...
      const tracks = [
        ...
        { id: "audio1", kind: "audio", name: "Audio", clips: audioClips2 }
      ];
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-overlay", onPointerDown: (e) => e.target === e.currentTarget && !running && onClose(), children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal wide", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "panel-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "✨ Auto-Edit" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn small", onClick: onClose, disabled: running, children: "✕" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-body", children: videos.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-hint", children: "Import some video first, then run Auto-Edit." }) : /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "insp-section", children: "Clips" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "ae-clips", children: videos.map((v) => /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "ae-clip", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("input", { type: "checkbox", checked: sel.has(v.id), onChange: () => toggle(v.id) }),
        v.thumbnail && /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: v.thumbnail, alt: "" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: v.name })
      ] }, v.id)) }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "insp-row", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Music" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { value: musicId, onChange: (e) => setMusicId(e.target.value), children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "", children: "None" }),
          audios.map((a) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: a.id, children: a.name }, a.id))
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "insp-row", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Style" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "btn-group", children: [
          ["talking", "Talking head"],
          ["montage", "Beat montage"],
          ["short", "Short-form 9:16"]
        ].map(([s, label]) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
      ...
      !sys?.whisper && style !== "montage" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tool-msg", children: "Whisper model missing — draft will skip captions." }),
      log.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("pre", { className: "ae-log", children: log.join("\n") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-actions", children: done ? /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn primary", onClick: onClose, children: "Open draft" }) : /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn primary", onClick: run, disabled: running || sel.size === 0, children: running ? "Analyzing…" : "Create draft" }) })
    ] }) })
  ] }) });
}
...
class Compositor {
  ...
  fbo;
  accum = 0;
  // index of the FBO currently accumulating
  w = 0;
  h = 0;
...
function textRasterFor(clip, w, h) {
  ...
  if (!entry || entry.key !== key) {
    ...
    if (textCache.size > 64) {
      ...
      if (first !== void 0) textCache.delete(first);
    }
  }
  return entry;
}
const active = (c, t) => t >= c.start && t < c.start + c.duration;
function renderFrame(comp, project, t, sources, outW, outH) {
  ...
  for (const track of project.tracks) {
    ...
    for (const clip of track.clips) {
      ...
      const media = clip.mediaId ? byId.get(clip.mediaId) : void 0;
      if (!media) continue;
      const src = clip.kind === "video" ? sources.getVideo(media, clip) : sources.getImage(media);
      if (!src) continue;
      comp.drawLayer({
        ownerId: (clip.bgRemoved && media.mattePath ? "mm:" : "m:") + media.id,
        source: src.source,
        dynamic: clip.kind === "video",
        contentKey: clip.kind === "video" ? void 0 : "static",
...
function seekTo(el, time) {
  const max = isFinite(el.duration) && el.duration > 0 ? el.duration - 1e-3 : time;
  const target = Math.max(0, Math.min(time, max));
  if (el.readyState >= 2 && Math.abs(el.currentTime - target) < 6e-3) return Promise.resolve();
...
function loadVideo(media, matte) {
  const el = document.createElement("video");
  el.muted = true;
  el.preload = "auto";
  const usable = matte ? media.mattePath : PLAYABLE_VCODECS.includes(media.vcodec || "") || !media.proxyPath ? media.path : media.proxyPath;
...
async function runFramePipeExport(project, settings, onProgress, cancel) {
  ...
  const videoEls = /* @__PURE__ */ new Map();
  const imageEls = /* @__PURE__ */ new Map();
  const mediaById = new Map(project.media.map((m) => [m.id, m]));
  const keyFor = (clip, media) => (clip.bgRemoved && media.mattePath ? "matte:" : "") + media.id;
    ...
    for (let i = 0; i < totalFrames; i++) {
      ...
      for (const track of project.tracks) {
        ...
        for (const clip of track.clips) {
          ...
          const media = mediaById.get(clip.mediaId);
          const el = media ? videoEls.get(keyFor(clip, media)) : void 0;
      ...
      const accepted = await api.export2Frame(buf.buffer);
      if (!accepted) break;
  ...
  if (result.ok) {
    ...
    result.encoderUsed = (start.encoder === "qsv" ? "h264_qsv (Quick Sync" : "libx264 (software") + ", frame pipe)";
  }
  onProgress(1);
  return result;
}
...
function ExportDialog({ onClose }) {
  ...
  const presets = reactExports.useMemo(() => {
    ...
    return [
      { short: 720, label: "720p", defaultBitrateK: 6e3 },
      { short: 1080, label: "1080p", defaultBitrateK: 12e3 },
      { short: 2160, label: "4K", defaultBitrateK: 35e3 }
  ...
  const [presetIdx, setPresetIdx] = reactExports.useState(1);
  const [fps, setFps] = reactExports.useState(project.fps);
  const [bitrateK, setBitrateK] = reactExports.useState(presets[1].defaultBitrateK);
  const [encoder, setEncoder] = reactExports.useState("auto");
  const [running, setRunning] = reactExports.useState(false);
  ...
  reactExports.useEffect(() => setPlaying(false), [setPlaying]);
  reactExports.useEffect(() => api.on("export:progress", (p) => setProgress(p)), []);
  ...
  const start = async () => {
    ...
    if (usePipe) {
      ...
      const textPngs = project.tracks.flatMap((t) => t.clips).filter((c) => c.kind === "text" && (c.text || "").trim().length > 0).map((c) => ({ clipId: c.id, dataUrl: renderTextPngDataUrl(c, project.width, project.height) }));
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-overlay", onPointerDown: (e) => e.target === e.currentTarget && !running && onClose(), children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "panel-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Export video" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn small", onClick: onClose, disabled: running, children: "✕" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-body", children: [
      !running && !result?.ok && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "insp-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Resolution" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "btn-group", children: presets.map((p, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "insp-value", children: [
            presets[presetIdx].w,
            "×",
            presets[presetIdx].h
          ] })
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "insp-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Frame rate" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "btn-group", children: [24, 30, 60].map((f) => /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "insp-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Bitrate" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "input",
            {
              ...
              step: 500,
              value: bitrateK,
              onChange: (e) => setBitrateK(Number(e.target.value) || 1e3)
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-value", children: "kbps" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "insp-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Encoder" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "btn-group", children: ["auto", "qsv", "x264"].map((enc) => /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              className: `btn small ${encoder === enc ? "active" : ""}`,
              onClick: () => setEncoder(enc),
              disabled: enc === "qsv" && sys ? !sys.qsv : false,
              children: enc === "auto" ? "Auto" : enc === "qsv" ? "Quick Sync" : "Software"
            },
            enc
          )) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "insp-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Duration" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-value", children: formatTime(dur) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "insp-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Engine" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "engine-note", children: usePipe ? "GPU frame pipe (chroma key / mask / adjust in use — slower, exact)" : "FFmpeg filtergraph (fast path)" })
        ] }),
        result && !result.ok && /* @__PURE__ */ jsxRuntimeExports.jsx("pre", { className: "export-error", children: result.error }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-actions", children: /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn primary", onClick: start, disabled: dur <= 0, children: dur <= 0 ? "Timeline is empty" : "Choose file & export" }) })
      ] }),
      running && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "insp-row", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: progress?.phase === "preparing" ? "Preparing…" : `Encoding… ${pct}%` }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-actions", children: /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn danger", onClick: cancel, children: "Cancel" }) })
      ] }),
      !running && result?.ok && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "export-done", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "export-done-icon", children: "✓" }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            "Exported with ",
            /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: result.encoderUsed })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "export-path", children: result.outPath })
        ] }),
...
function ProjectSettings() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "inspector-body", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "insp-section", children: "Project" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "insp-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Name" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          type: "text",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "insp-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Aspect" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "btn-group", children: aspects.map((a) => /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "insp-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "FPS" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "btn-group", children: [24, 30, 60].map((f) => /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-hint", children: "Select a clip to edit its properties." })
  ] });
}
function Inspector() {
  ...
  if (!loc) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "inspector", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "panel-header", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Inspector" }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(ProjectSettings, {})
    ] });
  }
  const clip = loc.clip;
  const t = clip.transform;
  const patchTransform = (p) => updateClip(clip.id, { transform: { ...t, ...p } }, false);
  const patchStyle = (p) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "inspector", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "panel-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: clip.kind === "text" ? "Text clip" : clip.kind === "audio" ? "Audio clip" : clip.kind === "adjust" ? "Adjustment layer" : "Clip" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn small danger", onClick: () => deleteClip(clip.id), children: "Delete" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "inspector-body", children: [
      clip.kind === "text" && clip.textStyle && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "insp-section", children: "Text" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "textarea",
          {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "insp-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Font" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "select",
            {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "insp-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Color" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "input",
            {
              type: "color",
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Outline" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "input",
            {
              type: "color",
      ...
      clip.kind === "video" && /* @__PURE__ */ jsxRuntimeExports.jsx(AiToolsSection, { clip }),
      clip.kind === "adjust" && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "insp-section", children: "Adjustment" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          Slider,
          {
            label: "Intensity",
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(ColorSection, { clip })
      ] }),
      (clip.kind === "video" || clip.kind === "image") && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(ChromaSection, { clip }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(MaskSection, { clip })
      ] }),
      (clip.kind === "video" || clip.kind === "audio") && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "insp-section", children: "Speed" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          Slider,
          {
            label: "Speed",
      ...
      (clip.kind === "video" || clip.kind === "image") && /* @__PURE__ */ jsxRuntimeExports.jsx(ColorSection, { clip }),
      (clip.kind === "video" || clip.kind === "audio") && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "insp-section", children: "Audio" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          Slider,
          {
            label: "Volume",
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "insp-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Muted" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "input",
            {
              type: "checkbox",
      ...
      loc.track.id === MAIN_TRACK_ID && loc.index < loc.track.clips.length - 1 && /* @__PURE__ */ jsxRuntimeExports.jsx(TransitionSection, { clip })
    ] })
  ] });
}
function TransformSection({ clip }) {
  ...
  const kfProps = [
    { prop: "x", label: "X", min: -0.5, max: 0.5, step: 5e-3 },
    { prop: "y", label: "Y", min: -0.5, max: 0.5, step: 5e-3 },
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "insp-section", children: "Transform" }),
    kfProps.map(({ prop, label, min, max, step }) => /* @__PURE__ */ jsxRuntimeExports.jsx(
      Slider,
      {
        label,
        value: t[prop],
    ...
    anyKf && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "insp-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn small", onClick: () => clearKeyframes(clip.id), children: "Clear all keyframes" })
    ] })
  ] });
}
function AiToolsSection({ clip }) {
  const project = useEditor((s) => s.project);
  const sys = useEditor((s) => s.sys);
  const updateClip = useEditor((s) => s.updateClip);
  ...
  const [bgRatio, setBgRatio] = reactExports.useState(0);
  const media = clip.mediaId ? project.media.find((m) => m.id === clip.mediaId) : void 0;
  reactExports.useEffect(
    () => api.on("bgremove:progress", (p) => {
      const { mediaId, ratio } = p;
      if (mediaId === media?.id) setBgRatio(ratio);
    }),
    [media?.id]
  );
  if (!media) return /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, {});
  ...
  const silenceCut = () => guard("silence", async () => {
  ...
  const sceneCut = () => guard("scenes", async () => {
  ...
  const captions = () => guard("captions", async () => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "insp-section", children: "AI tools" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "insp-row", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "btn-group wrap", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn small", disabled: !!busy, onClick: silenceCut, children: busy === "silence" ? "Analyzing…" : "✂ Silence cut" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn small", disabled: !!busy, onClick: sceneCut, children: busy === "scenes" ? "Analyzing…" : "🎬 Scene cut" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          className: "btn small",
          ...
          onClick: captions,
          children: busy === "captions" ? "Transcribing…" : "💬 Auto captions"
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          onClick: removeBg,
          children: busy === "bg" ? `Matting… ${Math.round(bgRatio * 100)}%` : clip.bgRemoved ? "👤 Restore BG" : "👤 Remove BG"
        }
      )
    ] }) }),
    msg && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tool-msg", children: msg })
  ] });
}
function ChromaSection({ clip }) {
  const updateClip = useEditor((s) => s.updateClip);
  const beginInteraction = useEditor((s) => s.beginInteraction);
  const ck = clip.chromaKey;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "insp-section", children: "Chroma key" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "insp-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Enabled" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          type: "checkbox",
          ...
          onChange: (e) => updateClip(clip.id, {
            chromaKey: e.target.checked ? { ...ck ?? defaultChromaKey(), enabled: true } : ck ? { ...ck, enabled: false } : void 0
          })
        }
      ),
      ck?.enabled && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Key color" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "input",
          {
            type: "color",
    ...
    ck?.enabled && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Slider, { label: "Similarity", value: ck.similarity, min: 0.01, max: 0.45, step: 5e-3, onCommitStart: beginInteraction, onChange: (v) => updateClip(clip.id, { chromaKey: { ...ck, similarity: v } }, false) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Slider, { label: "Smooth", value: ck.smoothness, min: 0, max: 0.4, step: 5e-3, onCommitStart: beginInteraction, onChange: (v) => updateClip(clip.id, { chromaKey: { ...ck, smoothness: v } }, false) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Slider, { label: "Spill", value: ck.spill, min: 0, max: 1, step: 0.01, onCommitStart: beginInteraction, onChange: (v) => updateClip(clip.id, { chromaKey: { ...ck, spill: v } }, false) })
    ] })
  ] });
}
function MaskSection({ clip }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "insp-section", children: "Mask" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "insp-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Type" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "btn-group", children: types.map((tp) => /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          className: `btn small ${(m?.type ?? null) === tp.value ? "active" : ""}`,
          onClick: () => updateClip(clip.id, {
            mask: tp.value ? { ...defaultMask(tp.value), ...m ? { ...m, type: tp.value } : {} } : void 0
          }),
          children: tp.label
        },
        tp.label
    ...
    m && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Slider, { label: "Center X", value: m.cx, min: 0, max: 1, step: 5e-3, onCommitStart: beginInteraction, onChange: (v) => patch({ cx: v }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Slider, { label: "Center Y", value: m.cy, min: 0, max: 1, step: 5e-3, onCommitStart: beginInteraction, onChange: (v) => patch({ cy: v }) }),
      m.type !== "linear" && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Slider, { label: "Width", value: m.w, min: 0.05, max: 2, step: 0.01, onCommitStart: beginInteraction, onChange: (v) => patch({ w: v }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(Slider, { label: "Height", value: m.h, min: 0.05, max: 2, step: 0.01, onCommitStart: beginInteraction, onChange: (v) => patch({ h: v }) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Slider, { label: "Feather", value: m.feather, min: 0, max: 0.5, step: 5e-3, onCommitStart: beginInteraction, onChange: (v) => patch({ feather: v }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Slider, { label: "Rotation", value: m.rotation, min: -180, max: 180, step: 1, onCommitStart: beginInteraction, onChange: (v) => patch({ rotation: v }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "insp-row", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Invert" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("input", { type: "checkbox", checked: m.invert, onChange: (e) => patch({ invert: e.target.checked }) })
      ] })
    ] })
  ] });
}
function ColorSection({ clip }) {
  ...
  const patch = (p) => updateClip(clip.id, { color: { ...color, ...p } }, false);
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "insp-section", children: "Color" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "insp-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Preset" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "btn-group wrap", children: FILTER_PRESETS.map((p) => /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          className: "btn small",
          onClick: () => {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(Slider, { label: "Exposure", value: color.exposure, min: -1, max: 1, step: 0.01, onCommitStart: beginInteraction, onChange: (v) => patch({ exposure: v }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(Slider, { label: "Contrast", value: color.contrast, min: -1, max: 1, step: 0.01, onCommitStart: beginInteraction, onChange: (v) => patch({ contrast: v }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(Slider, { label: "Saturation", value: color.saturation, min: -1, max: 1, step: 0.01, onCommitStart: beginInteraction, onChange: (v) => patch({ saturation: v }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(Slider, { label: "Warmth", value: color.temperature, min: -1, max: 1, step: 0.01, onCommitStart: beginInteraction, onChange: (v) => patch({ temperature: v }) })
  ] });
}
function TransitionSection({ clip }) {
  ...
  const setType = (type) => {
    updateClip(clip.id, {
      transitionAfter: type ? { type, duration: cur?.duration ?? 0.5 } : void 0
    });
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "insp-section", children: "Transition to next" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "insp-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "insp-label", children: "Type" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "btn-group", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: `btn small ${!cur ? "active" : ""}`, onClick: () => setType(null), children: "None" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
    ...
    cur && /* @__PURE__ */ jsxRuntimeExports.jsx(
      Slider,
      {
        label: "Duration",
        value: cur.duration,
...
function MediaPool() {
  ...
  const doImport = async () => {
    ...
    try {
      ...
      if (paths.length > 0) {
        const items = await api.ingest(paths);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "media-pool", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "panel-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Media" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn primary", onClick: doImport, disabled: importing, children: importing ? "Importing…" : "+ Import" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "media-grid", children: [
      media.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-hint", children: "Import video, audio or images to get started." }),
      media.map((m) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "media-item", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "media-thumb", children: [
          m.thumbnail ? /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: m.thumbnail, alt: "", draggable: false }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `thumb-placeholder t-${m.type}`, children: m.type === "audio" ? "♪" : m.type === "image" ? "🖼" : "🎬" }),
          m.duration > 0 && m.type !== "image" && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "media-dur", children: formatTime(m.duration).slice(0, 5) }),
          proxyPending(m.type, m.vcodec, m.proxyPath) && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "proxy-badge", children: "optimizing…" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "media-name", title: m.path, children: m.name }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "media-actions", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn small", onClick: () => addToTimeline(m.id), title: "Add to timeline", children: "+ Add" }),
          m.type !== "audio" && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn small", onClick: () => addOverlayClip(m.id), title: "Add as picture-in-picture overlay", children: "PiP" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn small danger", onClick: () => removeMedia(m.id), title: "Remove from project", children: "×" })
        ] })
      ] }, m.id))
    ] })
  ] });
}
function audioClips(project) {
  const byId = new Map(project.media.map((m) => [m.id, m]));
  const out = [];
  for (const track of project.tracks) {
    if (track.kind === "audio" && !track.muted) {
      for (const c of track.clips) out.push({ clip: c, media: c.mediaId ? byId.get(c.mediaId) : void 0 });
    }
  }
  return out;
}
...
function Preview() {
  ...
  const videoEls = reactExports.useRef(/* @__PURE__ */ new Map());
  const audioEls = reactExports.useRef(/* @__PURE__ */ new Map());
  const imageEls = reactExports.useRef(/* @__PURE__ */ new Map());
  const [glError, setGlError] = reactExports.useState(null);
  ...
  reactExports.useEffect(() => {
    ...
    const useMatte = (media, clip) => !!(clip?.bgRemoved && media.mattePath);
    const getVideoEl = (media, clip) => {
    ...
    const syncPlayable = (el, clip, t, isPlaying, gainMult) => {
      const speed = clip.speed || 1;
    ...
    const tick = () => {
      ...
      const dt = (now - lastNow) / 1e3;
      lastNow = now;
      const dur2 = projectDuration(s.project);
      let t = s.playhead;
      if (s.playing) {
        t = t + dt;
        if (t >= dur2) {
      ...
      const activePlayables = /* @__PURE__ */ new Set();
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "preview-panel", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "preview-stage", children: glError ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "empty-hint", children: [
      "Preview unavailable — WebGL2 failed: ",
      glError
    ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx(
      "canvas",
      {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "preview-controls", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn icon", title: "Go to start", onClick: () => setPlayhead(0), children: "⏮" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "timecode", children: [
        formatTime(playhead),
        " / ",
        formatTime(dur)
      ] })
...
function PlayheadLine() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "playhead", style: { left: playhead * pps } });
}
function Ruler({ width }) {
  ...
  for (let i = 0; i < count && i < 600; i++) {
    const t = i * interval;
    ticks.push(
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tick", style: { left: t * pps }, children: formatTime(t).slice(0, 5) }, i)
    );
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tl-ruler-ticks", children: ticks });
}
function Timeline() {
  const project = useEditor((s) => s.project);
  const pps = useEditor((s) => s.pps);
  ...
  const timeAt = reactExports.useCallback(
    (clientX) => {
  ...
  const snapTime = reactExports.useCallback((t, excludeClipId) => {
    const s = useEditor.getState();
    if (!s.snap) return t;
  ...
  const onRulerDown = (e) => {
    ...
    const move = (ev) => useEditor.getState().setPlayhead(timeAt(ev.clientX));
    const up = () => {
  ...
  const onClipDown = (e, clip, track) => {
    e.stopPropagation();
    e.preventDefault();
    const s = useEditor.getState();
    s.select(clip.id);
    const target = e.target;
    const mode = target.classList.contains("handle-l") ? "trim-l" : target.classList.contains("handle-r") ? "trim-r" : "move";
    drag.current = {
      mode,
      clipId: clip.id,
      trackId: track.id,
  ...
  const onDragMove = reactExports.useCallback((e) => {
    ...
    if (!loc) return;
    const media = d.orig.mediaId ? s.project.media.find((m) => m.id === d.orig.mediaId) : void 0;
    ...
    if (d.mode === "trim-l") {
      ...
      const minDelta = isMedia ? -d.orig.in / speed : loc.track.id === MAIN_TRACK_ID ? -3600 : -d.orig.start;
    ...
    if (targetRow && !trackAccepts(targetRow.track, d.orig)) targetRow = void 0;
  ...
  const onDragUp = reactExports.useCallback(() => {
    drag.current = null;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "timeline", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tl-toolbar", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn", onClick: splitAtPlayhead, title: "Split at playhead (S)", children: "✂ Split" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          className: "btn",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn", onClick: addTextClip, title: "Add a text clip (T)", children: "T Text" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          className: "btn",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "snap-toggle", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("input", { type: "checkbox", checked: snap, onChange: toggleSnap }),
        " Snap"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "spacer" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn icon", onClick: () => setPps(pps * 0.8), title: "Zoom out", children: "−" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          type: "range",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn icon", onClick: () => setPps(pps * 1.25), title: "Zoom in", children: "+" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tl-body", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tl-scroll", ref: scrollRef, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
          ...
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tl-ruler", onPointerDown: onRulerDown, children: /* @__PURE__ */ jsxRuntimeExports.jsx(Ruler, { width }) }),
                ...
                children: track.clips.map((clip) => {
                  ...
                  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
                    "div",
                    {
                      ...
                      children: [
                        media?.thumbnail && clip.kind !== "audio" && /* @__PURE__ */ jsxRuntimeExports.jsx(
                          "div",
                          {
                        ...
                        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "clip-label", children: clip.kind === "text" ? `T ${clip.text || ""}` : media?.name || clip.kind }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "handle handle-l" }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "handle handle-r" })
                      ]
                    },
                    clip.id
                  );
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(PlayheadLine, {})
          ]
        }
      ) })
    ] })
...
function App() {
  ...
  const [autoEditOpen, setAutoEditOpen] = reactExports.useState(false);
  reactExports.useEffect(() => {
    const onKey = (e) => {
      const el = e.target;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "app", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "topbar", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "spacer" }),
      sys && !sys.ffmpegFound && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "badge warn", title: "Place ffmpeg.exe and ffprobe.exe in tools\\ffmpeg\\", children: "FFmpeg missing" }),
      sys?.ffmpegFound && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "badge ok", title: sys.ffmpegVersion, children: sys.qsv ? "Quick Sync ✓" : "CPU encode" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn", onClick: () => setAutoEditOpen(true), children: "✨ Auto-Edit" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn primary", onClick: () => setExportOpen(true), children: "Export" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "workspace", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(MediaPool, {}),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Preview, {}),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Inspector, {})
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(Timeline, {}),
    exportOpen && /* @__PURE__ */ jsxRuntimeExports.jsx(ExportDialog, { onClose: () => setExportOpen(false) }),
    autoEditOpen && /* @__PURE__ */ jsxRuntimeExports.jsx(AutoEditDialog, { onClose: () => setAutoEditOpen(false) })
  ] });
}
const AUTOSAVE_MS = 3e3;
async function boot() {
  if (hasApi) {
    const [last, sys] = await Promise.all([api.loadLastProject(), api.sysInfo()]);
    if (last) useEditor.getState().init(last);
    useEditor.getState().setSys(sys);
    api.on("media:proxy-ready", (payload) => {
      const { mediaId, proxyPath } = payload;
    ...
    setInterval(() => {
      ...
      api.saveProject(s.project).then(() => {
